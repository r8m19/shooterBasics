...
public class PlayerMovementController : MonoBehaviour
{
    ...
    {
        rb.WakeUp();
        rb.velocity += Mathf.Sqrt(-2 * Physics.gravity.y * altitud) * Vector3.up;
        enElPiso = false;
    }
    ...
    void ChequearSalto()
    {
        if(Input.GetKeyDown(teclaSalto)){
            if(enElPiso){
                Saltar(altitudDeSalto);
            }
            else
            {
                if(jumpBuffer != null) StopCoroutine(jumpBuffer);
                jumpBuffer = StartCoroutine(BufferedJump());
            }
        }
    }
    ...
    IEnumerator BufferedJump(){
        float i = 0;
        while (i < jumpBufferTime && !enElPiso)
        {
